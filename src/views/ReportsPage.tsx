        ...
        const getAllReports = async () => {
            try {
                const response = await fetch(
                    `https://api.spaceflightnewsapi.net/v4/reports/?limit=10&offset=${(currentPage - 1) * 10}`
                );
                        ...
                        <div className='grid grid-cols-1 gap-3 px-3 md:px-10 lg:px-20 xl:px-60 mt-5'>
                            ...
                            <div className='flex flex-row justify-between px-1 mt-3' >
                                <BsArrowLeftSquareFill
                                    ...
                                    className={`w-6 h-6 cursor-pointer hover:translate-y-0.5 hover:text-orange-200 ${allReports.previous ? '' : 'opacity-30 cursor-default hover:text-white'}`}
                                />
                                <BsArrowRightSquareFill
                                    onClick={handleNext}
                                    className={`w-6 h-6 cursor-pointer hover:translate-y-0.5 hover:text-orange-200 ${allReports.next ? '' : 'opacity-30 cursor-default hover:text-white'}`}
                                /></div>
                        </div>
                    </>
                )}
